...
from src.dataset import ComplexDataset, generate_three_loader_v2
from src.parameter_paser import parse_args_area_1
...
def training(model,
    ...
    for i_epoch in range(num_epochs_t):
        ...
        for batch_idx, (data_batch, label_int_batch) in enumerate(dataloader_t):
        ...
        total_valid_loss = 0
        with torch.no_grad():
            for batch_idx, (data_batch, label_int_batc) in enumerate(valid_data_loader_t):
                data_batch = data_batch.to(device)

                label_pred_onehot = model(data_batch).cpu()             # [@, n_class]
                label_pred = torch.argmax(label_pred_onehot, dim=-1)    # [@]

                correct_count += (label_pred == label_int_batc).sum().item()
                total_count += label_pred_onehot.shape[0]
                total_valid_loss += loss_function(label_int_batc, label_pred_onehot).item()
...
def testing(model, 
    ...
    with torch.no_grad():
        ...
        # only one iteration
        for _, (data_batch, labels_int) in enumerate(dataloader_t):  # for all test samples
            data_batch = data_batch.to(device)

            label_pred_onehot = model(data_batch).cpu()              # [@, n_class]
            label_pred = torch.argmax(label_pred_onehot, dim=-1)     # [@]

            correct_count += (label_pred == labels_int).sum().item()
            total_count += label_pred_onehot.shape[0]

            acc = (correct_count * 1.0) / (total_count * 1.0) * 100
            precision = precision_score(labels_int, label_pred, average='weighted') * 100
            recall = recall_score(labels_int, label_pred, average='weighted') * 100
...
if __name__ == '__main__':
    ...
    args = parse_args_area_1(base_dir)
    print(f"\nUsing configuration file: {args.config}\n")

    input_dir = os.path.join(base_dir, "input")
    data_path = os.path.join(input_dir, args.data_name)

    output_dir = os.path.join(base_dir, "output")
    os.makedirs(output_dir, exist_ok=True)
    model_path_train = os.path.join(output_dir, args.save_model_name)
    ...
    phase = loaded['phase']
    label = loaded['label']
    complex_dataset = ComplexDataset(amplitude, phase, label)

    train_loader, valid_loader, test_loader = generate_three_loader_v2(complex_dataset, 
                                                            batch_size_t, 
                                                            frac_for_valid, 
                                                            frac_for_test)
